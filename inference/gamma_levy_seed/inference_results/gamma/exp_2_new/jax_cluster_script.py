...
if __name__ == "__main__":
    ...
    if envelope == 'exponential':
        TRUE_ENVELOPE_PARAMS = (0.25,)
        np.random.seed(seed = 2352929)

    elif envelope == 'gamma':
        TRUE_ENVELOPE_PARAMS = (1.5,1.)
        np.random.seed(seed = 11243925348)
        raise ValueError('sssgf')
    ...
    lags_list = ((1,3,5,10),(1,3,5),(1,3))#,(1,5,10),(1,5,10,15),(1,5,10,15,20)),(1,3,5,10,15),(1,3,5,10,20))
    n_values = (2000,1500,1000,750,500,250)#,1000, 2500, 5000)#,750,1000,1500)
    ...
    for lags_index in range(len(lags_list)):
        print('lags are', lags_list[lags_index])
        for n_index in range(len(n_values)):
    ...
    end_gmm = time.time()
    print('gmm fitting finished, time taken: ', (end_gmm - start_gmm)//60, ' minutes')      
    
    	        
    #fit the cl model
    for lags_index in range(len(lags_list)):
        start_current_lag = time.time()
        progress_file = open('proress_file.txt','w+')
        progress_file.write('lags_index is ' +str(lags_index) +'\n'),
        progress_file.close()
        
        
        for n_index in range(len(n_values)):
            print('lags_index is: ',lags_index)
            print('n_index is: ',n_index)
            progress_file = open('proress_file.txt','w+')
            progress_file.write('n_index is ' + str(n_index) + '\n')
            progress_file.close()
            ...
            for simulation_to_use in range(nr_simulations): 
                print('simulation ', simulation_to_use, ' / ',nr_simulations)
                progress_file = open('proress_file.txt','w+')
                progress_file.write('simulation_to_use is ' + str(simulation_to_use) + '\n')
                progress_file.close()
                ...
                try:
                    ...
                    try:
                        ...
                        print('simulation ', simulation_to_use, ' is very problematic')
        ...
        print('current lags time was: ', (end_current_lag - start_current_lag)//60)
        
    end_cl = time.time()
    print('cl fitting finished, time taken: ', (end_cl - end_gmm)//60, ' minutes')     
